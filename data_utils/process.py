...
ffmpeg_path = "/usr/bin/ffmpeg" # or "ffmpeg"

def extract_audio(path, out_path, sample_rate = 16000):
    print(f'[INFO] ===== extract audio from {path} to {out_path} =====')
    cmd = f'{ffmpeg_path} -i {path} -f wav -ar {sample_rate} {out_path}'
    os.system(cmd)
...
def extract_audio_features(path, mode = 'wav2vec'):
    ...
    if mode == 'wav2vec':
        cmd = f'python data_utils/asr.py --wav {path} --save_feats'
    else:  # deepspeech
...
def extract_images(path, out_path, fps = 25):
    ...
    cmd = f'{ffmpeg_path} -y -i {path} -vf "scale=trunc(iw/16)*16:trunc(ih/16)*16" -c:a copy -c:v h264 -crf 20 {temp_path}'
    print(cmd)
    return_code = subprocess.call(cmd, shell = True)
    assert return_code == 0

    cmd = f'{ffmpeg_path} -y -i {temp_path} -vf fps={fps} -qmin 1 -q:v 1 -start_number 0 {os.path.join(out_path, "%d.jpg")}'
    print(cmd)
...
def extract_landmarks(ori_imgs_dir):
    ...
    # save_path = os.path.join(ori_imgs_dir, 'lmk_3ddfa.npy')
    # os.system(f'python data_utils/landmark_3ddfav2.py --video_fp {opt.path} --save_path {save_path}')
    #
    # lmk_3ddfa = np.load(save_path)

    # for i, image_path in enumerate(tqdm.tqdm(natsorted(image_paths))):
    #     lands = lmk_3ddfa[i]
    #     np.savetxt(image_path.replace('jpg', 'lms'), lands, '%f')
...
def extract_head_mask(base_dir, ori_imgs_dir):
    print(f'[INFO] ===== extract head mask for {base_dir} =====')

    from scipy.ndimage import binary_erosion, binary_dilation

    # load bg
    os.makedirs(ori_imgs_dir.replace('ori_imgs', 'head_mask'), exist_ok = True)
    image_paths = glob.glob(os.path.join(ori_imgs_dir, '*.jpg'))
    ...
    for image_path in tqdm.tqdm(image_paths):
        ...
        seg = cv2.imread(image_path.replace('ori_imgs', 'parsing').replace('.jpg', '.png'))
        head_part = (seg[..., 0] == 255) & (seg[..., 1] == 0) & (seg[..., 2] == 0)
        head_part = np.stack([head_part] * 3, axis = -1) * 255
        cv2.imwrite(image_path.replace('ori_imgs', 'head_mask').replace('.jpg', '.png'), head_part.astype(np.uint8))

    print(f'[INFO] ===== extracted torso and gt images =====')


def extract_bg_with_torso(base_dir, ori_imgs_dir):
    print(f'[INFO] ===== extract head mask for {base_dir} =====')

    from scipy.ndimage import binary_erosion, binary_dilation
    bg = cv2.imread(os.path.join(base_dir, 'bc.jpg'))
    # load bg
    os.makedirs(ori_imgs_dir.replace('ori_imgs', 'bg_torso'), exist_ok = True)
    image_paths = glob.glob(os.path.join(ori_imgs_dir, '*.jpg'))

    for image_path in tqdm.tqdm(image_paths):
        print(image_path)
        org_img = cv2.imread(image_path.replace('ori_imgs', 'torso_imgs').replace('.jpg', '.png'), cv2.IMREAD_UNCHANGED)
        torso_mask = org_img[..., -1:] / 255
        _img = org_img[..., :3] * torso_mask + bg * (1 - torso_mask)

        cv2.imwrite(image_path.replace('ori_imgs', 'bg_torso'), _img.astype(np.uint8))

    print(f'[INFO] ===== extracted bg with torso images =====')
...
def save_mv_transforms(base_dir, ori_imgs_dir):
    print(f'[INFO] ===== save transforms =====')
    ...
    for split in range(2):
        ...
        for i in ids:
            frame_dict = dict()
            frame_dict['img_id'] = i.item()
            frame_dict['aud_id'] = i.item()
            frame_dict['exp'] = params_dict['exp'][[i]].numpy().tolist()
            frame_dict['id'] = params_dict['id'].numpy().tolist()
        ...
        with open(os.path.join(base_dir, 'mv_transforms_' + save_id + '.json'), 'w') as fp:
            json.dump(transform_dict, fp, indent = 2, separators = (',', ': '))

    print(f'[INFO] ===== finished saving transforms =====')


def extract_face_recon_3dmm(base_dir, ori_imgs_dir):
    print(f'[INFO] ===== extract face recon 3dmm info =====')
    ori_imgs_dir = os.path.abspath(ori_imgs_dir)
    print(ori_imgs_dir)
    os.system(f'cd data_utils/Deep3DFaceRecon &&'
              f' python run.py --name=facerecon --epoch=20 --img_folder={ori_imgs_dir}')
    print(f'[INFO] ===== finished extracting face recon 3dmm info =====')


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--path', type = str, help = "path to video file")
    parser.add_argument('--save_dir', type = str, help = "path to video file")
    ...
    if opt.task == -1 or opt.task == 8:
        extract_face_recon_3dmm(base_dir, ori_imgs_dir)
    ...
    # save transforms.json DynTet format
    if opt.task == -1 or opt.task == 11:
        save_mv_transforms(base_dir, ori_imgs_dir)

    if opt.task == -1 or opt.task == 12:
        extract_head_mask(base_dir, ori_imgs_dir)

    if opt.task == -1 or opt.task == 13:
        extract_bg_with_torso(base_dir, ori_imgs_dir)
